...
| `/jwa/full-chain-sop/` | `C:/Project/JWASystemv2/jwa-sop/sop/jwa-full-chain/sop.json` + `shots/` beside it. `jwa-sop` is a worktree of jwa-system **on `main`**, kept only for this; `deploy-site.ps1` fast-forwards it to `origin/main` before every build and refuses to build if it can't. **SOP changes land on main through PRs** — never edit `sop.json` in the `jwa-system` dev checkout (its branch trails main; building from it is what left main's ledger behind the page until 2026-09-23) | zyt pipeline. Registry id `jwa-system/jwa-full-chain` (draft); built with zyt-setup's `build-page.mjs` without `--no-ledger` since 2026-09-16, so its fix list is public (NCT's moved to the dashboard's Findings in Phase 4) |
